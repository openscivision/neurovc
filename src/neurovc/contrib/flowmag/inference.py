...
def inference(
    ...
    with torch.no_grad():
        ...
        for i in tqdm(range(1, len(frames_dataset))):
            ...
            for _ in range(num_recursion):
                ...
                frames = torch.stack([im0, pred[0, :, 0].unsqueeze(0)], dim=2).repeat(
                    num_device, 1, 1, 1, 1
                )
    ...
    if output_video:
        saved_frames = [
            (255 * img.squeeze().permute(1, 2, 0).flip([-1]).numpy()).astype(np.uint8)
            for img in results
        ]
        video_path = (
            str(save_dir / f"x{alpha}.mp4")
            if mask is None
            else str(save_dir / f"masked_x{alpha}.mp4")
        )
    ...
    # Save as image files
        ...
        save_dir = (
            save_dir / f"x{alpha}" if mask is None else save_dir / f"masked_x{alpha}"
        )
        save_dir.mkdir(exist_ok=True, parents=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="path to directory of frames to magnify",
    )
    parser.add_argument("--resume", type=str, help="path to checkpoint to resume from")
    parser.add_argument(
        "--save_name", type=str, required=True, help="name to save under"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="path to numpy mask, if None then no mask",
    )
    parser.add_argument(
        "--soft_mask",
        type=int,
        default=0,
        help="how much to soften mask. 0 is none, higher is more",
    )
    ...
    # Make frames dataset
    frames_dataset = (
        TrainingFramesDataset(args.frames_dir)
        if args.test_time_adapt
        else FramesDataset(args.frames_dir)
    )
