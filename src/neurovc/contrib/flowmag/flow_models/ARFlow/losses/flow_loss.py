...
class unFlowLoss(nn.modules.Module):
    ...
    def loss_photomatric(self, im1_scaled, im1_recons, occu_mask1):
        ...
        if self.cfg.w_ssim > 0:
            loss += [
                self.cfg.w_ssim * SSIM(im1_recons * occu_mask1, im1_scaled * occu_mask1)
            ]

        if self.cfg.w_ternary > 0:
            loss += [
                self.cfg.w_ternary
                * TernaryLoss(im1_recons * occu_mask1, im1_scaled * occu_mask1)
            ]

        return sum(item.mean() for item in loss) / occu_mask1.mean()
    ...
    def forward(self, output, target):
        ...
        for i, flow in enumerate(pyramid_flows):
            ...
            if i == 0:
                ...
                occu_mask1 = F.interpolate(
                    self.pyramid_occu_mask1[0], (h, w), mode="nearest"
                )
                occu_mask2 = F.interpolate(
                    self.pyramid_occu_mask2[0], (h, w), mode="nearest"
                )
            ...
            if self.cfg.with_bk:
                loss_warp += self.loss_photomatric(im2_scaled, im2_recons, occu_mask2)
        ...
        pyramid_warp_losses = [
            loss_value * weight
            for loss_value, weight in zip(pyramid_warp_losses, self.cfg.w_scales)
        ]
        pyramid_smooth_losses = [
            loss_value * weight
            for loss_value, weight in zip(pyramid_smooth_losses, self.cfg.w_sm_scales)
        ]
