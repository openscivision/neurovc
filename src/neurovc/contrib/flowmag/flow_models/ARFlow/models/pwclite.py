...
class PWCLite(nn.Module):
    def __init__(self, cfg):
        ...
        self.conv_1x1 = nn.ModuleList(
            [
                conv(192, 32, kernel_size=1, stride=1, dilation=1),
    ...
    def forward_2_frames(self, x1_pyramid, x2_pyramid):
        ...
        for level_idx, (x1, x2) in enumerate(zip(x1_pyramid, x2_pyramid)):
            # warping
            if level_idx == 0:
                x2_warp = x2
            else:
            ...
            x1_1by1 = self.conv_1x1[level_idx](x1)
            ...
            if level_idx == self.output_level:
                break
        if self.upsample:
            flows = [
                F.interpolate(
    ...
    def forward_3_frames(self, x0_pyramid, x1_pyramid, x2_pyramid):
        ...
        init_device = x1_pyramid[0].device
        flow = torch.zeros(
            b_size, 4, h_x1, w_x1, dtype=init_dtype, device=init_device
        ).float()

        for level_idx, (x0, x1, x2) in enumerate(
            zip(x0_pyramid, x1_pyramid, x2_pyramid)
        ):
            # warping
            if level_idx == 0:
                x0_warp = x0
            ...
            corr_relu_10, corr_relu_12 = (
                self.leakyRELU(corr_10),
                self.leakyRELU(corr_12),
            )
    ...
    def forward(self, x, with_bk=False):
        ...
        # for i in range(len(res_dict['flows_fw'])):
        # res_dict['flows_fw'][i] = res_dict['flows_fw'][i].cuda(0)
        return res_dict
