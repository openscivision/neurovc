...
class TrainFramework(BaseTrainer):
    def __init__(
        self, train_loader, valid_loader, model, loss_func, _log, save_root, config
    ):
    ...
    def _run_one_epoch(self):
        ...
        for i_step, data in enumerate(self.train_loader):
            ...
            flows = [
                torch.cat([flo12, flo21], 1) for flo12, flo21 in zip(flows_12, flows_21)
            ]
            ...
            if self.cfg.run_atst:
                ...
                st_res = (
                    self.sp_transform(deepcopy(s)) if self.cfg.run_st else deepcopy(s)
                )
            ...
            if self.cfg.run_ot:
                ...
                # random crop images
                img_pair, flow_t, occ_t = random_crop(
                    img_pair, flow_ori, 1 - noc_ori, self.cfg.ot_size
                )

                # slic 200, random select 8~16
                if self.cfg.ot_slic:
                    img2 = img_pair[:, 3:]
                    seg_mask = run_slic_pt(
                        img2,
                        n_seg=200,
                        compact=self.cfg.ot_compact,
                        rd_select=[8, 16],
                        fast=self.cfg.ot_fast,
                    ).type_as(img2)  # Nx1xHxW
            ...
            if self.i_iter % self.cfg.print_freq == 0:
                istr = (
                    "{}:{:04d}/{:04d}".format(self.i_epoch, i_step, self.cfg.epoch_size)
    ...
    def _validate_with_gt(self):
        ...
        for i_set, loader in enumerate(self.valid_loader):
            ...
            for i_step, data in enumerate(loader):
                ...
                error_meters.update([err.item() for err in es], img_pair.size(0))
                ...
                if i_step % self.cfg.print_freq == 0 or i_step == len(loader) - 1:
                    self._log.info(
                        "Test: {0}[{1}/{2}]\t Time {3}\t ".format(
                            i_set, i_step, self.cfg.valid_size, batch_time
                        )
            ...
            all_error_names.extend(
                ["{}_{}".format(name, i_set) for name in error_names]
            )
