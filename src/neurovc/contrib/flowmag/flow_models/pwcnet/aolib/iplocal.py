...
def run_reload():
    ...
    for name, m in list(sys.modules.items()):
        if (
            m
            and relative_module(m)
            and (name not in excludes)
            and (not hasattr(m, "__no_autoreload__"))
        ):
            imp.reload(m)
...
class Parallel:
    ...
    def per_machine_subset(self, n_per_machine):
        ...
        target_subset = ut.flatten(
            [ut.sample_at_most(ts, n_per_machine) for ts in list(targets.values())]
        )
    ...
    def send_var_fs(self, var_name, val):
        ...
        ut.save(fname, val)
        res = self.view().execute(
            'import util as ut; ut.wait_for_file("%s", 30); %s = ut.load("%s")'
    ...
    def map_sync(self, f, xs, vars={}, imports=[], do_reload=True, do_reset=False):
        # if do_reset:
...
def init(do=True, local=False):
    if do:
        ...
        if local:
            ...
            config_file = None
            # config_file = '/scratch/aho/ipython-home/profile_default/security/ipcontroller-engine.json'
            # config_file = '/scratch/aho/ipython-home/profile_default/.ipython/security/ipcontroller-engine.json'
...
def test_partition():
    def f(x):
        ...
        return sum(iputil.partition(2, 1).map_sync(lambda x: x, list(range(10))))

    from . import iputil

    iputil.map_sync(lambda x: x, list(range(10000)))
    assert sum(iputil.partition(2, 0).map_sync(f, list(range(1000)))) == 1000 * sum(
        range(10)
    )
