...
def parse_color(c):
    if isinstance(c, tuple) or isinstance(c, np.ndarray):
        return c
...
def colors_from_input(color_input, default, n):
    ...
    if color_input is None:
        expanded = [default] * n
    elif (isinstance(color_input, tuple)) and list(map(type, color_input)) == [
        int,
        int,
        int,
    ]:
    ...
    expanded = list(map(parse_color, expanded))
    return expanded
...
def draw_rects(
    ...
    if texts is None:
        texts = [None] * len(rects)
    if line_widths is None:
        line_widths = [None] * len(rects)

    def check_size(x, s):
        ut.check(x is None or len(x) == len(rects), "%s different size from rects" % s)
...
def draw_rects_scale(
    sc, im, rects, outlines=None, fills=None, texts=None, text_colors=None
):
    scaled_rects = []
    for r in rects:
        r = np.array(r)
...
def draw_pts(im, points, colors=None, width=1, texts=None):
    ...
    return draw_rects(
        im, rects, fills=colors, outlines=[None] * len(points), texts=texts
    )
...
def luminance(im):
    if len(im.shape) == 2:
        ...
        return np.uint8(
            np.round(0.2989 * im[:, :, 0] + 0.587 * im[:, :, 1] + 0.114 * im[:, :, 2])
        )
...
def test_draw_text():
    ...
    show(
        [
            draw_text(
                im, ["hello", "world"], [(100, 200), (0, 0)], [(255, 0, 0), (0, 255, 0)]
            ),
            draw_text(
                im,
                ["hello", "world"],
...
def resize(im, scale, order=3, hires=False):
    ...
    # interpret scale as dimensions; convert integer size to a fractional scale
        ...
        ((scale[0] is None) or isinstance(scale[0], int))
        and ((scale[1] is None) or isinstance(scale[1], int))
        ...
        new_scale = [
            float(dims[0] + 0.4) / im.shape[0],
            float(dims[1] + 0.4) / im.shape[1],
        ]
    ...
    elif isinstance(scale[0], float) and isinstance(scale[1], float):
        new_scale = scale
    ...
    if hires:
        ...
        sz = list(
            map(int, (scale_param[1] * im.shape[1], scale_param[0] * im.shape[0]))
        )
        ...
        if (
            (np.ndim(scale) != 0)
            and isinstance(scale[0], int)
            and isinstance(scale[1], int)
        ):
...
def test_resize():
    ...
    assert resize(im, (2.0, 0.5, "t")).shape[:2] == (88, 22)
...
def blur(im, sigma):
    if np.ndim(im) == 2:
        ...
        return np.concatenate(
            [
                scipy.ndimage.filters.gaussian_filter(im[:, :, i], sigma)[
                    :, :, np.newaxis
                ]
...
def weighted_add(src, dst, x, y, src_weight, dst_weight, opt=None):
    ...
    sx = dx - x
    sy = dy - y
    dst[dy : dy + dh, dx : dx + dw] = (
        dst[dy : dy + dh, dx : dx + dw] * dst_weight
        + src[sy : sy + dh, sx : sx + dw] * src_weight
    )
...
def luminance_rgb(im):
    return rgb_from_gray(luminance(im))
...
def rotate(img, angle, fill=0):
    ...
    X2 = np.array(
        np.round(
            img.shape[1] / 2 + np.cos(angle) * (X - r / 2) - np.sin(angle) * (Y - r / 2)
        ),
        dtype=int,
    )
    Y2 = np.array(
        np.round(
            img.shape[0] / 2 + np.sin(angle) * (X - r / 2) + np.cos(angle) * (Y - r / 2)
        ),
        dtype=int,
    )
    good = ut.logical_and_many(X2 >= 0, X2 < img.shape[1], Y2 >= 0, Y2 < img.shape[0])
    out = fill + np.zeros(
        (r, r) if img.ndim == 2 else (r, r, img.shape[2]), dtype=img.dtype
    )
    out[Y[good], X[good]] = img[Y2[good], X2[good]]
    T = np.dot(
        np.dot(
            ut.rigid_transform(np.eye(2), [img.shape[1] / 2, img.shape[0] / 2]),
...
def map_img(f, im, dtype=None, components=None):
    new_im = np.zeros(
        im.shape if components is None else im.shape + (components,),
...
def merge_ims(srcs, pts_or_rects, bg, opt=None):
    ...
    for src, r in zip(srcs, pts_or_rects):
        r = ut.int_tuple(r)
        x, y = r[:2]
        ...
        if np.ndim(src) == 3 and src.shape[2] == 4:
            counts[dy : dy + dh, dx : dx + dw] += (
                np.array(src[sy : sy + dh, sx : sx + dw, 3], "d") / 255.0
            )
        else:
...
def stack_meshgrid(xs, ys, dtype="l"):
    ...
    return np.array(
        np.concatenate([x[..., np.newaxis], y[..., np.newaxis]], axis=2), dtype=dtype
    )


def sub_img_pad(im, xxx_todo_changeme, oob=0):
    ...
    dst[(sy - y) : (sy - y) + sh, (sx - x) : (sx - x) + sw] = im[
        sy : sy + sh, sx : sx + sw
    ]
    return dst


def sub_img_reflect(im, xxx_todo_changeme1):
    (x, y, w, h) = xxx_todo_changeme1
    x, y, w, h = list(map(ut.iround, [x, y, w, h]))
    yy, xx = np.mgrid[y : y + h, x : x + w]
    vals = np.uint8(
        lookup_bilinear(im, xx.flatten(), yy.flatten(), order=0, mode="reflect")
    )
...
# def uncompress(s):
#   return from_pil(Image.open(BytesIO(s)))


def uncompress(s):
    # return from_pil(Image.open(BytesIO(s.encode())))
    if isinstance(s, str):
        return from_pil(Image.open(BytesIO(s.encode("latin-1"))))
    else:
        return from_pil(Image.open(BytesIO(s)))
...
def lookup_bilinear(im, x, y, order=1, mode="constant", cval=0.0):
    ...
    if np.ndim(im) == 2:
        ...
        return np.concatenate(
            [
                scipy.ndimage.map_coordinates(im[:, :, i], yx, order=order, mode=mode)[
                    :, np.newaxis
                ]
                for i in range(im.shape[2])
            ],
...
def map_helper(xxx_todo_changeme2):
    (xs, i, order, mode) = xxx_todo_changeme2
    return scipy.ndimage.map_coordinates(xs, np.array([i]), order=order, mode=mode)[
        :, np.newaxis
    ]
            ...
            return np.concatenate(
                [
                    scipy.ndimage.map_coordinates(
                        xs[:, j], np.array([i]), order=order, mode=mode
                    )[:, np.newaxis]
