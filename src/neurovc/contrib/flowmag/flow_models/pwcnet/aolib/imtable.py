...
PUBLIC_URL = "http://owens-gpu1.eecs.umich.edu/tab"
...
def get_url():
    ...
    if name == "zzz":
        ...
        return "http://people.eecs.berkeley.edu/~owens/tab"
    elif name in ["owens-gpu1"]:
        return "http://owens-gpu1.eecs.umich.edu/tab"
    elif name in ["owens-gpu2"]:
        return "http://owens-gpu2.eecs.umich.edu/tab"
    else:
...
def get_www_path():
    ...
    if name == "zzz":
        ...
        # return '/home/ahowens/tab'
        return "/n/owens-data1/mnt/Big1/ahowens/tab"
    elif name in ["owens-gpu2"]:
        # return '/home/ahowens/tab'
        return "/n/owens-data1/mnt/Big1/ahowens/tab"
    else:
...
def cycle_html(im_fnames_table, group_name, start):
    im_list_js = js_list([js_list(list(map(ut.quote, row))) for row in im_fnames_table])
    element_id = str(random.random())
    ...
    start_repr = repr(list(start))
    return (
        f'<img src = "{default}" id = "{element_id}" onmouseover = \'curr_cycle_group = "{group_name}";\''
        f" onclick = 'cycle(\"{group_name}\", 1, 1, event.shiftKey, event.ctrlKey)', oncontextmenu = 'cycle(\"{group_name}\", 1, -1, event.shiftKey, event.ctrlKey); return false;'"
        f' onload = \'register_cycle("{group_name}", "{element_id}", {im_list_js}, {start_repr});\'>'
    )
...
def path_from_im(x, output_dir, img_encoding=".png"):
    if isinstance(x, tuple) and (x[0] in ("img", "img_mv")):
        x = list(x)
        mv_im = x[0] == "img_mv"
        if isinstance(x[1], str):
            x[1] = os.path.abspath(x[1])
        im = x[1]
    elif isinstance(x, tuple) and (x[0] in ("img-png",)):
...
def html_from_cell(x, output_dir):
    if isinstance(x, str):
        ...
        if x[0] in ("img", "img_mv", "img-png"):
            ...
            if len(x) >= 3 and isinstance(x[2], dict):
                opts = x[2]
            else:
            ...
            if len(im_table) == 0 or not isinstance(im_table[0], list):
                im_table = [im_table]
...
def html_from_rows(table, output_dir):
    ...
    for row in table:
        ...
        if isinstance(row, tuple) and row[0] == "break":
...
def sync_with_fps(ims, timestamps, target_fps, max_lag_sec=0.06):
    ...
    while i < len(ims):
        ...
        def video_time():
            return (len(vid) - 1) * (1.0 / target_fps)
...
class Cycle:
    ...
    def make_html(self, output_dir):
        if len(self.ims) == 0 or not isinstance(self.ims[0], list):
            ims = [self.ims]
