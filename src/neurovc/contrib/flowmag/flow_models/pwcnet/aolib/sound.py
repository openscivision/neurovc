...
def vis_specgram_at_times(
    ...
    if not compress_ims:
        mapper = par.map if hasattr(par, "map") else map
        ims = [ig.uncompress(res) for res in mapper(f, times)]
    else:
        ims = list(map(f, times))
    return ims
...
class Sound:
    ...
    def __getitem__(self, *args):
        return Sound(
            self.times.__getitem__(*args), self.rate, self.samples.__getitem__(*args)
        )
    ...
    def slice_time(self, t1, t2=None):
...
def resample_snd(xxx_todo_changeme):
    (snd, sr) = xxx_todo_changeme
...
def concat_sounds(snds, par=1):
    ...
    if ut.ndistinct(ut.mapattr(snds).rate) == 1:
        ...
        if par:
            ...
            new_snds = list(map(resample_snd, [(snd, sr) for snd in snds]))
