...
for strOption, strArgument in getopt.getopt(
    ...
    if strOption == "--pairs" and strArgument != "":
        arguments_strModel = strArgument  # which model to use
        ...
        self.load_state_dict(
            {
                ...
                for strKey, tenWeight in torch.hub.load_state_dict_from_url(
                    ...
                    + arguments_strModel
                    + ".pytorch",
                    file_name="pwc-" + arguments_strModel,
                ).items()
            }
        )
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        raise RuntimeError("Usage: run_batch.py <first_image> <second_image>")
    file1, file2 = sys.argv[1], sys.argv[2]

    tenFirst = torch.FloatTensor(
        numpy.ascontiguousarray(
            numpy.array(PIL.Image.open(file1))[:, :, ::-1]
            .transpose(2, 0, 1)
    ...
    tenSecond = torch.FloatTensor(
        numpy.ascontiguousarray(
            numpy.array(PIL.Image.open(file2))[:, :, ::-1]
            .transpose(2, 0, 1)
            .astype(numpy.float32)
            * (1.0 / 255.0)
        )
    ...
    with open(arguments_strOut, "wb") as f:
        np.save(f, np.array(tenOutput.numpy().transpose(1, 2, 0), numpy.float32))
