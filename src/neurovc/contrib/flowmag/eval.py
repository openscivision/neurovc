...
def test(args, model, save_dir):
    ...
    # Make loss function
    dotlist = [
        f"train.flow_model={args.flow_model}",
        f"train.flow_model_type={args.flow_model_type}",
        "train.raft_iters=20",
    ]
    ...
    with torch.no_grad():
        for idx, (frames, info_data) in enumerate(tqdm(testloader)):
            ...
            if (
                args.alpha > config.train.alpha_high
                and np.sqrt(args.alpha) <= config.train.alpha_high
            ):
                our_alpha = np.sqrt(args.alpha)
                frames_ = frames.clone()
                for _ in range(num_recursion):
                    pred = model(frames_, alpha=our_alpha)
                    frames_ = torch.stack([frames[:, :, 0], pred[:, :, 0]], dim=2).to(
                        device
                    )
    ...
    print(
        f"Avg motion error: {Motion_error.avg:.04f}, std: {Motion_error.std:.04f}, standard error: {Motion_error.se:.04f} @ alpha={args.alpha}"
    )
    print(
        f"Avg mag error: {Mag_error.avg:.04f}, std: {Mag_error.std:.04f}, standard error: {Mag_error.se:.04f} @ alpha={args.alpha}"
    )
    ...
    with open(save_file, "a+") as f:
        ...
        f.write(
            f"\tAvg motion error: {Motion_error.avg:.04f}, std: {Motion_error.std:.04f}, standard error: {Motion_error.se:.04f}\n"
        )
        f.write(
            f"\tAvg mag error: {Mag_error.avg:.04f}, std: {Mag_error.std:.04f}, standard error: {Mag_error.se:.04f}\n"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--alpha", type=float, required=True, help="amount to magnify motion"
    )
