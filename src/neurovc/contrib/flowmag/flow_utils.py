...
class ARFlow(nn.Module):
    def __init__(self):
        ...
        chkpt_path = (
            ...
            / "flow_models/ARFlow/checkpoints/KITTI15/pwclite_ar.tar"
        )
...
class PWC(nn.Module):
    ...
    def forward(self, im1, im2):
        ...
        tenFlow = 20.0 * torch.nn.functional.interpolate(
            input=self.flowNet(tenPreprocessedFirst, tenPreprocessedSecond),
...
def normalize_flow(flow):
    ...
    flow = -1 + 2.0 * flow / (-1 + size)[:, None, None]
