...
def motion_statistics(data_root, loader, dataset_name, split):
    ...
    saved_path = os.path.join(
        data_root, "flow_data/{}_{}.json".format(dataset_name, split)
    )
    ...
    with torch.no_grad():
        for i, (images, image_paths) in enumerate(tqdm.tqdm(loader)):
            ...
            imgs = [
                cv2.imread(get_absolute_path(path, data_root)).astype(float)
                for path in paths
            ]
            flow_data[i] = {
...
def filtering_vis(
    data_root, dataset_name, split, flow_data_path, threshold_dict, mse_threshold
):
    ...
    print(
        "number of the selected videos: {}/{}, from {} original folders".format(
            len(included_data), len(filtered_data), len(set(original_folders))
        )
    )

    with open(
        os.path.join(
            data_root, "flow_data/{}_{}_filtered.json".format(dataset_name, split)
        ),
        "w",
    ) as f:
    ...
    with open(
        os.path.join(
            data_root, "flow_data/{}_{}_included.json".format(dataset_name, split)
        ),
        "w",
    ) as f:
    ...
    print(
        "saving html to {}".format(
            os.path.join(data_root, "{}_{}_vis".format(dataset_name, split))
        )
    )
    html = HTML(
        os.path.join(data_root, "{}_{}_vis".format(dataset_name, split)), "subset"
    )
    ...
    html.add_header(
        "dataset: {}, split: {}, threshold: {}, {} (MSE)".format(
            dataset_name, split, str(threshold_dict), mse_threshold
        )
    )
    html.add_header(
        "filtered videos, {}/{}".format(
            len(filtered_data), len(filtered_data) + len(included_data)
        )
    )
    ims = []
    txts = []
    ...
    for n in filtered_vis:
        paths = [
            get_absolute_path(path, data_root) for path in filtered_data[n]["paths"]
        ]
        gif_path = save_gif(
            paths,
            os.path.join(data_root, "{}_{}_vis".format(dataset_name, split), "images"),
        )
        ims.append(gif_path)
    ...
    html.add_header(
        "included videos, {}/{}".format(
            len(included_data), len(filtered_data) + len(included_data)
        )
    )
    ims = []
    txts = []
    ...
    for n in included_vis:
        paths = [
            get_absolute_path(path, data_root) for path in included_data[n]["paths"]
        ]
        gif_path = save_gif(
            paths,
            os.path.join(data_root, "{}_{}_vis".format(dataset_name, split), "images"),
