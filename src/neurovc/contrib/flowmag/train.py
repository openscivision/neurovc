...
os.environ["CUDA_VISIBLE_DEVICES"] = ",".join(
    [str(i) for i in range(config.train.ngpus)]
)
...
writer = SummaryWriter(
    log_dir=os.path.join(config.log.log_dir, f"{now_str}-{config.name}", "runs"),
    comment=f".{config.name}",
)
config.save_dir = Path(config.log.log_dir) / f"{now_str}-{config.name}"
config.save_dir.mkdir(exist_ok=True, parents=True)
logging.basicConfig(
    filename=config.save_dir / "logs.txt",
    filemode="w",
    ...
    for cur_iter, data in enumerate(tqdm(trainloader)):
        ...
        if (cur_iter + 1) % config.log.print_freq == 0:
            ...
            for loss_name, meter in metrics.items():
                ...
                meter.reset()
            log_str = (
                f"[Epoch {epoch_num}] [Iter {cur_iter}/{len(trainloader)}] "
                + " | ".join(log_strs)
            )
...
for epoch_num in range(start_epoch + 1, config.train.num_epochs + 1):
    ...
    torch.save(chkpt, save_path / "latest.pth")
