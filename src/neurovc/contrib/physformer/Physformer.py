...
class ViT_ST_ST_Compact3_TDC_gra_sharp(nn.Module):
    def __init__(
        ...
        # gt, gh, gw = t // ft, h // fh, w // fw  # number of patches; kept for reference
        # seq_len = gh * gw * gt  # retained for reference if sequence length is needed later

        # Patch embedding    [4x16x16]conv
        self.patch_embedding = nn.Conv3d(
            dim, dim, kernel_size=(ft, fh, fw), stride=(ft, fh, fw)
        )
    ...
    def forward(self, x, gra_sharp):
        ...
        Trans_features2, Score2 = self.transformer2(
            Trans_features, gra_sharp
        )  # [B, 4*4*40, 64]
        Trans_features3, Score3 = self.transformer3(
            Trans_features2, gra_sharp
        )  # [B, 4*4*40, 64]
        ...
        features_last = Trans_features3.transpose(1, 2).view(
            b, self.dim, t // 4, 4, 4
        )  # [B, 64, 40, 4, 4]
